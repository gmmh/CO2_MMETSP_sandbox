...
num.na =apply(m,1,function(x){length(which(is.na(x)))}) #trim orthogroups found in <50% of samples
cut = which(num.na > ncol(m)/2)
m = m[-cut,]
dim(m)
#write new file
write.table(m,paste0(dir,"merged_counts_all.txt"))

#Cut table down to shared "core" orthologous groups
cut <- which(is.na(apply(m,1,mean)))
m.core = m[-cut,]
dim(m.core)
write.table(m.core,paste0(dir,"merged_counts_core.txt"))
